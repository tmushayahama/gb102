var mentorshipWeblinksCtrl = function (
 ...
 vm.mentorshipWeblinksSrv = new MentorshipWeblinksSrv();
 ...
 vm.createMentorshipWeblink = function (data) {
  vm.mentorshipWeblinksSrv.createMentorshipWeblink(data).then(function (response) {
   vm.weblinkFormDisplay = false;
   vm.newMentorshipWeblinkData = angular.copy(vm.defaultMentorshipWeblinkData);
   vm.mentorshipWeblinksCopy = angular.copy(vm.mentorshipWeblinksSrv.mentorshipWeblinks);
  }, function (response) {
 ...
 vm.editMentorshipWeblink = function (data) {
  vm.mentorshipWeblinksSrv.editMentorshipWeblink(data).then(function (response) {
   vm.weblinkFormDisplay = false;
   vm.newMentorshipWeblinkData = angular.copy(vm.defaultMentorshipWeblinkData);
   vm.mentorshipWeblinksCopy = angular.copy(vm.mentorshipWeblinksSrv.mentorshipWeblinks);
  }, function (response) {
 ...
 vm.revertMentorshipWeblink = function (mentorshipWeblink, mentorshipWeblinkCopy) {
  ...
  /*
   $filter('filter')
   (vm.mentorshipWeblinksSrv.mentorshipWeblinks, {id: mentorshipWeblinkId}, true)[0]
   = angular.copy($filter('filter')
   (vm.mentorshipWeblinksCopy, {id: mentorshipWeblinkId}, true)[0]);
   if (mentorshipWeblink.length && mentorshipWeblinkCopy.length) {
   // vm.mentorshipWeblinksSrv.mentorshipWeblinks angular.copy(vm.mentorshipWeblinksCopy);
   }
   */
 };
 ...
 $scope.$watch(angular.bind(this, function () {
  ...
  vm.doneCount = vm.mentorshipWeblinksSrv.mentorshipWeblinks.length - vm.remainingCount;
 ...
 //--------init------
 vm.mentorshipWeblinksSrv.getMentorshipWeblinks(vm.mentorshipId);
};
