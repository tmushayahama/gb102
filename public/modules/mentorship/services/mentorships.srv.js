var mentorshipsSrv = function ($http, $q) {
 ...
 MentorshipsSrv.prototype.getUserMentorships = function (userId) {
  var self = this;
  var deferred = $q.defer();
  //self.mentorships = [];
  $http.get('/api/mentorships/user/' + userId + '/all').success(function (data) {
   //self.mentorships = data;
   self.deferredHandler(data, deferred);
  }).error(function (data) {
   self.deferredHandler(data, deferred, 'Unknown error');
 ...
 MentorshipsSrv.prototype.getMentorshipRequestOptions = function (mentorshipId) {
  var self = this;
  var deferred = $q.defer();
  $http.get('/api/mentorship/' + mentorshipId + '/requestoptions').success(function (data) {
   self.deferredHandler(data, deferred);
  }).error(function (data) {
   self.deferredHandler(data, deferred, 'Unknown error');
  });
