(function ()
{
 ...
 /** @ngInject */
 function msNoteFormController(level_categories, $scope, $rootScope, NotesService, msUtils, $mdDialog, $timeout, LabelsService)
 {
  var MsNoteForm = this;
  var notes = NotesService.data;

  //Data
  MsNoteForm.selectedApp = [];
  MsNoteForm.privacy = level_categories.privacy;
  MsNoteForm.selectedPrivacy = {};
  MsNoteForm.type = '';
  ...
  MsNoteForm.selectApp = selectApp;
  MsNoteForm.selectPrivacy = selectPrivacy;
  ...
  function selectPrivacy(id, title) {
   MsNoteForm.selectedPrivacy.id = id;
   MsNoteForm.selectedPrivacy.title = title;
  }
  ...
  function create()
  {
   ...
   // Set default values
   MsNoteForm.note.parentComponentId = $scope.parentComponentId;
   MsNoteForm.note.typeId = MsNoteForm.selectedApp.id;
   MsNoteForm.note.privacyId = MsNoteForm.selectedPrivacy.id;

   // Add the note
   NotesService.createNote(angular.copy(MsNoteForm.note)).then(function (response) {
 ...
 function msNoteFormDirective()
 {
  return {
   ...
   templateUrl: 'src/app/main/apps/notes/directives/ms-note-form/ms-note-form.html',
